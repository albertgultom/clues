...
<div class="head-follow">
	<div class="avatar-follow follow-list">
		<a href="{{ url('user') }}/{{ $a->follower->username}}">
			<img src="{{ $a->follower->avatar == '' ? asset('images/default_pic.png') : asset('storage/avatar/'.$a->follower->avatar.'') }}" class="responsive-img" alt="">
		</a>
	</div>
	...
	<div class="button-follow right">
		@if ($a->follower->id != Auth::user()->id)
		@php($count = 0)
		@foreach (Auth::user()->followings as $b)
		@if ($a->follower->id == $b->following_id)
		@php($count = 1)
		@endif
		@endforeach
		@if ($count == 1)
		<button data-id="{{$a->follower->id}}" class="btn btn-default blue-clues follower">Following</button>
		@else
		<button data-id="{{$a->follower->id}}" class="btn btn-default blue-clues follower">Follow</button>
		@endif
		@endif
	</div>
</div>
